        ...
        try {
            ...
            foreach ($this->productList as $item) {
                if (!isset($item['quantity']) || $item['quantity'] <= 0) {
                    throw new \Exception("Cantidad inválida para el producto: {$item['product_name']}");
                }
            ...
            $totalAmount = collect($this->productList)->sum('total_amount');
            $receivedAmount = $this->payment_type_id == 2
                ? ($this->dollar_amount * $this->exchangeRate)
                : $this->cordoba_amount;

            if ($receivedAmount < $totalAmount) {
                throw new \Exception("El monto recibido es insuficiente. Faltan C\$" . number_format($totalAmount - $receivedAmount, 2));
            }
